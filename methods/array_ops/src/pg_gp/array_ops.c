...
/******* internal functions ********/
/*
 * This function returns the matrix-vector product.
 */
PG_FUNCTION_INFO_V1(__array_mdot);
Datum
__array_mdot(PG_FUNCTION_ARGS){
    if (PG_ARGISNULL(0)) { PG_RETURN_NULL(); }
    if (PG_ARGISNULL(1)) { PG_RETURN_NULL(); }
    ...
    int *lbs = (int *) palloc(ndims * sizeof(int));
    int i = 0;
    if (dims2[i] % dims1[i] != 0 || lbs1[i] != lbs2[i]) {
        ereport(ERROR, (errcode(ERRCODE_ARRAY_SUBSCRIPT_ERROR),
                        errmsg("size of array 2 must be divisible by size of array 1"),
                        errdetail("Arrays with range [%d,%d] and [%d,%d] for dimension %d are not compatible for operations.",
                                  lbs1[i], lbs1[i] + dims1[i], lbs2[i], lbs2[i] + dims2[i], i)));
    }
    dims[i] = dims2[i] / dims1[i];
    lbs[i] = lbs1[i];
    int nitems = ArrayGetNItems(1, dims);
    int nitems1 = ArrayGetNItems(1, dims1);
    int nitems2 = ArrayGetNItems(1, dims2);
    ...
    for (i = 0; i < nitems; ++i) {
        ...
        for (int j = 0; j < nitems1; ++j) {
            ...
            float8 e1 = datum_float8_cast(elt1, element_type   );
            float8 e2 = datum_float8_cast(elt2, element_type   );

            sum += e1*e2;
        }
        *resultp = float8_datum_cast(sum, element_type);
        resultp ++;
    }

    // construct return result
    ArrayType *res = construct_md_array(result,
                                 NULL,
                                 ndims,
                                 dims,
                                 lbs,

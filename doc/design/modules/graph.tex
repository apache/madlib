...
of vertices in the $message$ table, and are not already visited in the past.
...
iteration, will have one or more parents, based on how many vertices in
...
The $message$ table is then updated with the contents of $toupdate$ table,
except that only one parent is recorded for each newly found neighboring
vertices if it has more than one parent. The row with the smallest vertex id
of all its parents is recorded in the $message$ table as the newly found
neighboring vertex. The content of this updated $message$ is then copied
to the $out$ table, and this process continues until $message$ table is
empty.

...
docker pull madlib/postgres_9_6:latest
# Assuming git clone of incubator-madlib has been done, launch a container with the volume mounted
docker run -d --name madlib -v incubator-madlib:/incubator-madlib madlib/postgres_9.6:latest
...
docker exec madlib -c 'mkdir /incubator-madlib/build ; cd /incubator-madlib/build ; cmake .. ; make ; make install'
